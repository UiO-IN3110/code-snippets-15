...
def solve(t0, t1, dt, n, m, u_initial, f, nu, verbose=False):
    ...
    t = t0 + Nt*dt                              #end time
    code = r"""
    double *temp, *u_init = u, *u_other=u_new;  /*set pointers to u and u_new for later use*/
    int th_id, nthreads;
    int i, j, it, m_start, m_end;
    #pragma omp parallel private(th_id, nthreads, it, i, j, m_start, m_end, u, u_new, temp)
    {
        u = u_init;
        u_new = u_other;
